...
# -------------------------
# Barangay Images Gallery (Pre-loaded images)
# -------------------------
st.markdown("---")
st.subheader("🏞️ Barangay Evacuation Centers")

# Pre-loaded barangay images data
barangay_images_data = {
    "Poblacion": "1761751229523.jpg",
    "Cabugao": "1761751235696.jpg", 
    "Campo": "1761751239923.jpg",
    "Dugsangon": "1761751243631.jpg",
    "Payapag": "1761751250847.jpg",
    "Pongtud": "1761751254113.jpg",
    "Sto. Rosario": "1761751256993.jpg"
}
...
def display_image_from_file(filename):
    try:
        ...
        st.warning(f"⚠️ Image file '{filename}' not found.")
        return None

# Display all barangay images in a grid
st.markdown("### 🖼️ Barangay Evacuation Center Gallery")

cols = st.columns(3)
for i, (barangay, filename) in enumerate(barangay_images_data.items()):
    with cols[i % 3]:
        st.markdown(f"**{barangay}**")
        image_bytes = display_image_from_file(filename)
        if image_bytes:
            st.image(image_bytes, caption=f"Evacuation Centers - {barangay}", use_container_width=True)
        else:
            st.info(f"No image available for {barangay}")

# -------------------------
# Additional Image Upload Functionality
# -------------------------
st.markdown("---")
st.subheader("📤 Upload Additional Barangay Images")

# Folder and CSV setup for additional uploads
...
def delete_barangay_image(barangay):
    ...
    if not row.empty:
        ...
        df.to_csv(BARANGAY_IMG_FILE, index=False)
        st.success(f"🗑️ Deleted uploaded image for {barangay} successfully!")
    else:
        st.warning("⚠️ No uploaded image found for that barangay.")

# Barangay list for uploads
barangay_list = ["Poblacion", "Cabugao", "Cambuayon", "Campo", "Dugsangon", "Pautao", "Payapag", "Sto. Rosario", "Pongtud"]

# Upload form for additional images
with st.form("upload_barangay_form"):
    barangay = st.selectbox("Select Barangay", barangay_list)
    uploaded_file = st.file_uploader("Choose an image", type=["jpg", "jpeg", "png"])
    upload_submit = st.form_submit_button("Upload Additional Image")
    ...
    if upload_submit and uploaded_file:
        ...
        st.success(f"✅ Additional image for {barangay} uploaded successfully!")

# Display uploaded images
st.markdown("### 📸 Uploaded Barangay Images")
uploaded_df = load_barangay_images()
if uploaded_df.empty:
    st.info("No additional images uploaded yet. Upload barangay images above.")
else:
    cols = st.columns(3)
    for i, row in uploaded_df.iterrows():
        ...
        with cols[i % 3]:
            if os.path.exists(file_path):
                st.image(file_path, caption=f"Uploaded - {barangay}", use_container_width=True)
                delete_btn = st.button(f"🗑️ Delete {barangay}", key=f"delete_{barangay}")
                if delete_btn:
                    delete_barangay_image(barangay)
                    st.rerun()
            else:
                st.warning(f"⚠️ Missing uploaded file for {barangay}")
